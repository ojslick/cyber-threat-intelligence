...
export class ToolsListComponent extends ServerList implements OnInit, OnDestroy {
  ...
  details(event, { id, name }) {
    ...
    const navigation = this.threatContextNavigationService.getNavigation(name, `tools/${id}`);
  ...
  getDetailsLink(id) {
    return `/dashboard/organizations/${this.orgId}/modules/${this.moduleId}/threat_context/tools/${id}`;
  }
