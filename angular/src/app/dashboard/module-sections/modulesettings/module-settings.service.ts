...
import { regexSpace, regexUrlSpaces, regexWithOutQuotes } from 'app/utils/regexps';

@Injectable()
export class ModuleSettingsService {
  settings;
  actualSetting: any;
  actualSettingSubject: BehaviorSubject<any>;
  ...
  constructor(private httpUtils: HttpUtilsService) {
    this.actualSettingSubject = new BehaviorSubject<any>(this.actualSetting);
    this.defaultDiacriticsRemovalMap = this.defaultDiacriticsRemovalMap.concat(this.lowercaseDiacriticsRemovalMap);
    this.settings = {
      credential: [
        domainObj,
        ipObj,
        {
          ...
          texts: {
            ...
            description:
              'Insert your emails (e.g. bar@fo.oo, foo@b.ar). You can also add multiple emails (one per line).',
            list: 'Current email'
          }
        },
        adVancedFiltersObj,
      ...
      data_leakage: [
        ...
        domainObj,
        {
          ...
          texts: {
            ...
            tooltip: 'Invalid format',
            description:
              'Names or parts of a file name e.g. blueliv_confidential.docx e.g. security_report_blueliv.pdf',
  ...
  getSettings(orgId, moduleId, moduleName) {
    return observableOf(this.settings[moduleName]);
  }

  getActualSetting() {
    return this.actualSettingSubject;
  }

  setActualSetting(newSetting) {
    this.actualSettingSubject.next(newSetting);
  }
  ...
  getUnclassifiedAssets(orgId, moduleId) {
    return this.httpUtils.get(`${path}/organization/${orgId}/module/${moduleId}/credentials/counterByProfile`);
  }
}
