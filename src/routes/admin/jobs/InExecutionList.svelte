<script lang="ts">
  ...
  import type { InExecutionJob } from '$lib/types/admin';
  import { getHumanReadableDate, scrollToTop } from '$lib/utils/functions';
  ...
  const headers: DataTableHeader[] = [
    ...
    { key: 'enqueuedAt', value: 'ENQUEUED AT', sort: false, display: getHumanReadableDate },
    { key: 'startTime', value: 'START TIME', sort: false, display: getHumanReadableDate }
  ];

  let loading = false;
  let rows: InExecutionJob[] = [];
  let totalItems = 0;
  let page = 1;
  ...
  $: getJobsInExecution(page, pageSize, sortKey, sortDirection, search);
  ...
  async function refresh() {
    await getJobsInExecution(page, pageSize, sortKey, sortDirection, search);
  }

  async function getJobsInExecution(
    page: number,
    pageSize: number,
    sortKey: string,
    ...
    const response = await client.admin.getJobsInExecution(page, pageSize, search, sortKey, ascending);
    ...
    <Toolbar>
      <ToolbarContent>
        <div class="flex justify-between items-center w-full">
          <Button on:click={refresh} kind="ghost" disabled={loading} icon={Renew}>Refresh Jobs In Execution</Button>
          <ToolbarSearch placeholder="Search for jobs in execution" persistent class="max-w-[300px]" bind:value={searchValue} />
        </div>
      </ToolbarContent>
    </Toolbar>
  </DataTable>
