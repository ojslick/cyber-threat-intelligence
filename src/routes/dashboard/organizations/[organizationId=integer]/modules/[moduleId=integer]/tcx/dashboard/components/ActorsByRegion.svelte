<script lang="ts">
  ...
  // I have no "region", so I'll use "sophistication"
  function groupByRegion(actors: Actor[]): ChartData[] {
    const tmp: Record<string, number> = {};
    actors.forEach((actor) => {
      if (!tmp[actor.sophistication]) {
        tmp[actor.sophistication] = 1;
      } else {
        tmp[actor.sophistication]++;
      }
    });
